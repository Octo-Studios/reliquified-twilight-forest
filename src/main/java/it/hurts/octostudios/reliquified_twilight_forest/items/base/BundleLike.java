...
public interface BundleLike {
    ...
    default boolean tryInsert(Player player, ItemStack bundleStack, ItemStack toInsert) {
        List<ItemStack> notMutable = bundleStack.get(RTDataComponent.BUNDLE_LIKE_CONTENTS);
    ...
    default @NotNull List<ItemStack> getContents(ItemStack stack) {
        return stack.getOrDefault(RTDataComponent.BUNDLE_LIKE_CONTENTS, List.of());
    }
    ...
    default void setContents(Player player, ItemStack stack, List<ItemStack> contents) {
        ...
        stack.set(RTDataComponent.BUNDLE_LIKE_CONTENTS, contents.stream().filter(itemStack -> !itemStack.isEmpty()).toList());
