...
public class RTItems {
    ...
    public static final Function<Item, Item> CHARMS = item -> {
        if (item == TFItems.CHARM_OF_LIFE_1.get()) {
            return RTItems.BROKEN_CHARM_OF_LIFE_1.get();
        }
        if (item == TFItems.CHARM_OF_LIFE_2.get()) {
            return RTItems.BROKEN_CHARM_OF_LIFE_2.get();
        }
        if (item == TFItems.CHARM_OF_KEEPING_1.get()) {
            return RTItems.BROKEN_CHARM_OF_KEEPING_1.get();
        }
        if (item == TFItems.CHARM_OF_KEEPING_2.get()) {
            return RTItems.BROKEN_CHARM_OF_KEEPING_2.get();
        }
        if (item == TFItems.CHARM_OF_KEEPING_3.get()) {
            return RTItems.BROKEN_CHARM_OF_KEEPING_3.get();
        }
        return Items.AIR;
    };
